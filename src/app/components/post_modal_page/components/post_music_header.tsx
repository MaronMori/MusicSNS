...
export const Post_music_header = ({renewUserSongs, setRenewUserSongs, onBack, songSelectPage, setSongSelectPage, songPostPage}) => {
        ...
        if (currentTime > expireTime) {
            const refreshToken = sessionStorage.getItem("spotifyRefreshToken")
                ...
                const response = await fetch("api/spotifyAPI", {
                    ...
                    // トークンと現在の時刻＋有効期限を保存
                    currentTime = new Date().getTime(); // 現在の時刻をミリ秒で取得
                    const expiresIn = data.expiresIn; // アクセストークンの有効期限（秒）
                    const newExpireTime = currentTime + expiresIn * 1000; // 有効期限の時刻をミリ秒で計算
    ...
    return (
        <div className={"border-b-2 border-black flex justify-between pb-2"}>
            ...
            {!songSelectPage ?
                (!renewUserSongs ? (<Button type={"button"} onClick={(e) => handleRenewSongs(e)}>
                    <AutorenewIcon />
                </Button>) : <CircularProgress />) : <div className="opacity-0">Back</div>}
            {/* 透明なスペーサー */}
        </div>
    )
}