...
const ManageSongPlayerContext = createContext<ManageSongPlayerContextType>(defaultValue);


export const ManageSongPlayerProvider = ({children}) => {
    const [isPlayingTimelineContent, setIsPlayingTimelineContent] = useState(defaultValue.isPlayingTimelineContent);
    const [previewUrl, setPreviewUrl] = useState(defaultValue.previewUrl);
    const audioRef = useRef<HTMLAudioElement | null>(null);

    const value = {audioRef, isPlayingTimelineContent, setIsPlayingTimelineContent, previewUrl, setPreviewUrl}
